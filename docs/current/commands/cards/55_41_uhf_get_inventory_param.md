...
- Status: SUPPORTED
- Reason: PDF Ver.1.17 lists this command in section 7.4.3. AI/RAG organizes device, ROM, parameter, impact, response, and recovery conditions instead of deciding arbitrary non-use.
...
- Parameter constraints: Follow the field definitions in PDF section 7.4.3.
...
- ACK command: Use the command-specific ACK structure described in PDF section 7.4.3.
- Data length: See PDF section 7.4.3.
...
- Command-specific NACK notes: Check PDF section 7.4.3 and section 7.6 together.
...
- Command format source:
  - PDF 7.4.3
- ACK response source:
  - PDF 7.4.3 ACK response
- NACK response source:
  - PDF 7.6 common NACK; PDF 7.4.3 NACK response
- Device/ROM support source:
  - PDF 6.2.2
- RAM/FLASH impact source:
  - PDF 7.4.3; docs/current/12_RAM_FLASH_IMPACT_MATRIX.md
- RF / carrier / antenna safety source:
  - PDF 3.1; PDF 7.4.3; docs/current/13_RF_SAFETY_AND_CARRIER_RULES.md
- Traceability status:
  - TRACE_COMPLETE
- Notes:
  - No completed Hex
...
- Required prior checks:
  - Stage 0完了、対象機種/ROM、パラメータ、ログ保存先
- Required parameters:
...
- Required log fields:
  - device_series, product_type, rom_version, parameter_summary, ack_summary, timeout_ms, elapsed_ms
- Expected response type:
  - ACK/NACK/timeout/no-response
- Recovery note:
  - 影響範囲を確認し、必要に応じて設定戻しを記録する。
