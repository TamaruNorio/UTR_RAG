...
- PDF section: 7.4.16
- Command name: リーダライタ動作モードの書き込み
- Category: リーダライタ設定
- Command byte: 4Eh
- Detail command byte: 00h / 10h
- Subcommand byte: -
...
- Reason: PDF Ver.1.17 lists this command in section 7.4.16. AI/RAG organizes device, ROM, parameter, impact, response, and recovery conditions instead of deciding arbitrary non-use.
...
- Parameter constraints: Follow the field definitions in PDF section 7.4.16.
...
- ACK command: Use the command-specific ACK structure described in PDF section 7.4.16.
- Data length: See PDF section 7.4.16.
...
- Command-specific NACK notes: Check PDF section 7.4.16 and section 7.6 together.
...
- Command format source:
  - PDF 7.4.16
- ACK response source:
  - PDF 7.4.16 ACK response
- NACK response source:
  - PDF 7.6 common NACK; PDF 7.4.16 NACK response
- Device/ROM support source:
  - PDF 6.2.2
- RAM/FLASH impact source:
  - PDF 7.4.16; docs/current/12_RAM_FLASH_IMPACT_MATRIX.md
