...
- PDF section: 7.3.9
- Command name: チップバージョンの読み取り
- Category: リーダライタ制御
- Command byte: 55h
- Detail command byte: 90h
- Subcommand byte: 00h / 01h
...
- Reason: PDF Ver.1.17 lists this command in section 7.3.9. AI/RAG organizes device, ROM, parameter, impact, response, and recovery conditions instead of deciding arbitrary non-use.
...
- Parameter constraints: Follow the field definitions in PDF section 7.3.9.
...
- ACK command: Use the command-specific ACK structure described in PDF section 7.3.9.
- Data length: See PDF section 7.3.9.
...
- Command-specific NACK notes: Check PDF section 7.3.9 and section 7.6 together.
...
- Command format source:
  - PDF 7.3.9
- ACK response source:
  - PDF 7.3.9 ACK response
- NACK response source:
  - PDF 7.6 common NACK; PDF 7.3.9 NACK response
...
- Verification stage:
  - Stage 0: Stage 0/1 read-only verification
- Initial status:
...
- Latest result status:
  - REAL_DEVICE_PASS_WITH_NOTES
- Latest result notes:
  - v014 executed firmware-version subcommand 00h only and returned chip firmware 2.052 / chip name UR201.
  - Serial-number subcommand 01h was not executed.
