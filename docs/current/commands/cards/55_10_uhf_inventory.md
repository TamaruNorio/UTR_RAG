...
RFタグのEPC(UII)をInventoryする。

## 3. Usage status

- Status: SUPPORTED_WITH_PARAMETERS
- Reason: PDF Ver.1.17 lists this command in section 7.5.1. AI/RAG organizes device, ROM, parameter, impact, response, and recovery conditions instead of deciding arbitrary non-use.
...
- Parameter constraints: Follow the field definitions in PDF section 7.5.1.
...
- ACK command: Use the command-specific ACK structure described in PDF section 7.5.1.
- Data length: See PDF section 7.5.1.
...
- Command-specific NACK notes: Check PDF section 7.5.1 and section 7.6 together.
...
- Command format source:
  - PDF 7.5.1
- ACK response source:
  - PDF 7.5.1 ACK response
- NACK response source:
  - PDF 7.6 common NACK; PDF 7.5.1 NACK response
- Device/ROM support source:
  - PDF 6.2.3
- RAM/FLASH impact source:
  - NEEDS_RAM_FLASH_TRACE
- RF / carrier / antenna safety source:
  - PDF 3.1; PDF 7.5.1; docs/current/13_RF_SAFETY_AND_CARRIER_RULES.md
- Traceability status:
