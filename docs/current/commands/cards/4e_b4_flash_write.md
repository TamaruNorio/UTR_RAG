...
- PDF section: 7.4.29
- Command name: FLASH設定値の書き込み(1バイトアクセス)
- Category: リーダライタ設定
- Command byte: 4Eh
- Detail command byte: B4h
- Subcommand byte: -
...
- Reason: PDF Ver.1.17 lists this command in section 7.4.29. AI/RAG organizes device, ROM, parameter, impact, response, and recovery conditions instead of deciding arbitrary non-use.
...
- Parameter constraints: Follow the field definitions in PDF section 7.4.29.
...
- ACK command: Use the command-specific ACK structure described in PDF section 7.4.29.
- Data length: See PDF section 7.4.29.
...
- Command-specific NACK notes: Check PDF section 7.4.29 and section 7.6 together.
...
- Command format source:
  - PDF 7.4.29
- ACK response source:
  - PDF 7.4.29 ACK response
- NACK response source:
  - PDF 7.6 common NACK; PDF 7.4.29 NACK response
- Device/ROM support source:
  - PDF 6.2.2
- RAM/FLASH impact source:
  - PDF 7.4.29; docs/current/12_RAM_FLASH_IMPACT_MATRIX.md
...
- Required prior checks:
  - Stage 0完了、設定前値、FLASH/永続影響、復旧手順、承認、作業ログ保存先
- Required parameters:
...
- Required log fields:
  - ram_flash_impact, recovery_required, pre_read_required, parameter_summary, ack_summary, nack_error_code_1-4
- Expected response type:
  - ACK/NACK/timeout/no-response
- Recovery note:
  - FLASH/永続設定に関わるため、事前読み取り、復旧手順、承認条件を記録する。
