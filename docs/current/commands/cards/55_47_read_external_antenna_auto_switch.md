...
command_name: "外部アンテナ自動切替設定の読み取り"
command_byte: "55h"
detail_command: "47h"
subcommand: null
...
このカードは、UTR-S201 シリーズ通信プロトコル説明書 Ver.1.17 に記載された **外部アンテナ自動切替設定の読み取り** を、AIとの実装・レビュー・検証で参照しやすくするための整理資料です。

- PDF章番号: `7.4.10`
- コマンド分類: リーダライタ設定
- 確認区分: `read-only`
- 操作レベル: 読み取り専用
- コマンドバイト: `55h` / 詳細コマンド: `47h` / サブコマンド: `null`
- 確認状態: `REAL_DEVICE_VERIFIED_WITH_NOTES`
- 結果状態: `REAL_DEVICE_PASS_WITH_NOTES`

## 2. 目的

このコマンドの目的は、**外部アンテナ自動切替設定の読み取り** です。
...
以下は、公式PDF `7.4.10` のコマンド表、レスポンス表、条件、例を、AIがカード単体で参照できるように転記したものです。値一覧、byte数、条件、ACK/NACK条件を省略せず、この節を実装時の一次確認に使ってください。

```text
7.4.10   外部アンテナ自動切替設定の読み取り
  外部アンテナの自動切替設定を読み取るコマンドです。
...
ラベル名      バイト数                                     内容
                     アンテナ出力 1 外部接続アンテナ数 ［初期値：01h］
             1
                      ※0～32 を指定（使用しない場合は 0）
                     アンテナ出力 2 外部接続アンテナ数 ［初期値：00h］
             1
                      ※0～32 を指定（使用しない場合は 0）
                     アンテナ出力 3 外部接続アンテナ数 ［初期値：00h］
             1
                      ※0～32 を指定（使用しない場合は 0）
                     アンテナ出力 4 外部接続アンテナ数 ［初期値：00h］
             1
                      ※0～32 を指定（使用しない場合は 0）
データ部
                     アンテナ出力 5 外部接続アンテナ数 ［初期値：00h］
             1
                      ※0～32 を指定（使用しない場合は 0）
                     アンテナ出力 6 外部接続アンテナ数 ［初期値：00h］
             1
                      ※0～32 を指定（使用しない場合は 0）
                     アンテナ出力 7 外部接続アンテナ数 ［初期値：00h］
             1
                      ※0～32 を指定（使用しない場合は 0）
                     アンテナ出力 8 外部接続アンテナ数 ［初期値：00h］
             1
                      ※0～32 を指定（使用しない場合は 0）
 ETX         1       03h
 SUM         1       SUM 値（「5.3 SUM の計算方法」参照）
  CR         1       0Dh
※取得した値の説明は、「7.4.26 外部アンテナ自動切替設定の書き込み」をご参照ください。
  ...
  ［コマンド／レスポンス例］
     (例)工場出荷時設定のリーダライタ UTR-SUN02V-8CH／UTR-SUN02-8CH で、電源投入直
        後に「外部アンテナ自動切替設定の読み取り」の FLASH データを読み取りした場合


    • コマンド
       02 00 55 03 47 02 00 03 A6 0D
    • レスポンス
       02 00 30 0C 47 02 00 02 01 00 00 00 00 00 00 00 03 8D 0D
         データ種類                    数値／パラメータ                           レスポンス列
       読み取り対象              FLASH データ                          02
                           アンテナ自動切替：[使用しない]
       動作パラメータ                                                02
                           外部アンテナ ID 出力：[使用する]
                           ANT1 = 1 , ANT2 = 0 , ANT3 = 0
       アンテナ出力別
                           ANT4 = 0 , ANT5 = 0 , ANT6 = 0     01 00 00 00 00 00 00 00
       外部接続アンテナ数
                           ANT7 = 0 , ANT8 = 0
```
...
| 項目 | 内容 |
|---|---|
| 代表コマンドHex | `02 00 55 03 47 02 00 03 A6 0D` |
| 代表ACK Hex | `02 00 30 0C 47 02 00 02 01 00 00 00 00 00 00 00 03 8D 0D` |
| ACK構造 | `02 ADR 30 0C 47 PARAM_KIND RSV OP ANT1 ANT2 ANT3 ANT4 ANT5 ANT6 ANT7 ANT8 03 SUM 0D` |
| ACK `LEN` | `0Ch`。DATA部は12byte固定 |
| DATA[0] | `47h`: 詳細コマンド |
| DATA[1] | `PARAM_KIND`: `00h`=コマンドモード、`01h`=自動読み取りモード、`02h`=FLASH |
| DATA[2] | `RSV`: 予約。通常 `00h` |
| DATA[3] | `OP`: bit0=アンテナ自動切替、bit1=外部アンテナID出力 |
| DATA[4]..DATA[11] | `ANT1`..`ANT8`: 各アンテナ出力の外部接続アンテナ数。範囲は0-32 |

このコマンドは8CH仕様リーダライタで有効です。読戻し値は配線・外部アンテナ構成の解釈に使えますが、`55_37` の書き込みへ自動投入しないでください。外部アンテナID出力ONの場合、RFタグ応答のアドレスbyteがリーダIDではなくアンテナ番号として振る舞う点も併せて確認してください。

### 8.2 NACK例（フォーマットエラーの例）

| 項目 | 内容 |
|---|---|
| 代表NACK Hex | `02 00 31 0A 47 44 00 00 00 00 00 00 00 00 03 CB 0D` |
...
| `CMD=30h` かつ `DATA[0]` が `47h` またはPDF該当節の応答識別子 | `ACK` | 対象コマンド `55h 47h` の成功応答としてPDF該当節を読む |
| `CMD=6Ch` | `RF_TAG_DATA` | 自動読み取り中の非同期応答候補。通常ACKとは分離する |

対象識別子: コマンド `55h` / 詳細 `47h` / サブ `なし`。


#### ACK/データ部offset
成功ACK `CMD=30h` のDATA先頭は、原則として `47h` またはPDF該当節の応答識別子として扱います。
...
| DATA offset | フィールド | 解釈 |
|---:|---|---|
| 0 | `error_source` | 原則として対象コマンドの詳細識別子。対象: `55h 47h` |
