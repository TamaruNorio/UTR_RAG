...
| 項目 | 内容 |
|---|---|
| 代表ACK Hex | `02 00 30 01 57 03 8D 0D` |
| ACKデータ部の先頭 | `57h` |
| 注意 | PDF 7.3.3 のACKレスポンス表でフィールドを確認してください。 |
...
| `CMD=30h` かつ `DATA[0]` が `57h` またはPDF該当節の応答識別子 | `ACK` | 対象コマンド `4Eh 57h` の成功応答としてPDF該当節を読む |
...
#### ACK/データ部offset
成功ACK `CMD=30h` のDATA先頭は、原則として `57h` またはPDF該当節の応答識別子として扱います。
