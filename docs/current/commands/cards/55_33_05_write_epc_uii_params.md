...
- PDF section: 7.4.25
- Command name: EPC(UII)関連パラメータの書き込み
- Category: リーダライタ設定
- Command byte: 55h
...
EPC(UII)関連パラメータを書き込む。

## 3. Usage status

- Status: SUPPORTED_WITH_PARAMETERS
- Reason: PDF Ver.1.17 lists this command in section 7.4.25. AI/RAG organizes device, ROM, parameter, impact, response, and recovery conditions instead of deciding arbitrary non-use.
...
- Parameter constraints: Follow the field definitions in PDF section 7.4.25.
...
- ACK command: Use the command-specific ACK structure described in PDF section 7.4.25.
- Data length: See PDF section 7.4.25.
...
- Command-specific NACK notes: Check PDF section 7.4.25 and section 7.6 together.
...
- Command format source:
  - PDF 7.4.25
- ACK response source:
  - PDF 7.4.25 ACK response
- NACK response source:
  - PDF 7.6 common NACK; PDF 7.4.25 NACK response
- Device/ROM support source:
  - PDF 6.2.2
- RAM/FLASH impact source:
  - PDF 7.4.25; docs/current/12_RAM_FLASH_IMPACT_MATRIX.md
