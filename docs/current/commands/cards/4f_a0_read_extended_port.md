...
| 項目 | 内容 |
|---|---|
| ACK構造 | `02 ADR 30 LEN A0 <READ_DATA...> 03 SUM 0D` |
| ACKデータ部の先頭 | `A0h` |
| 後続データ | PDF 7.4.12 のACKレスポンス表で定義される読戻し値。バイト数と意味はコマンドごとに異なります。 |
...
| `CMD=30h` かつ `DATA[0]` が `A0h` またはPDF該当節の応答識別子 | `ACK` | 対象コマンド `4Fh A0h` の成功応答としてPDF該当節を読む |
