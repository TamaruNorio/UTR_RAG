...
このカードは、UTR-S201 シリーズ通信プロトコル説明書 Ver.1.17 に記載された **拡張ポート値の書き込み** を、AIとの実装・レビュー・検証で参照しやすくするための整理資料です。

- PDF章番号: `7.4.28`
- コマンド分類: リーダライタ設定
- 確認区分: `settings-change`
- 操作レベル: write/configuration
- コマンドバイト: `4Eh` / 詳細コマンド: `A0h` / サブコマンド: `null`
- 確認状態: `REAL_DEVICE_VERIFIED_WITH_NOTES`
- 結果状態: `REAL_DEVICE_PASS_WITH_NOTES`

## 2. 目的

このコマンドの目的は、アンテナポート8CH仕様リーダライタの拡張ポート1〜8のうち、指定したビットに対応するポート値だけをLow/Highへ書き換えることです。

このカードには、PDF 7.4.28に記載されたコマンドフィールド、ポート指示ビット、ポート設定値ビット、ACK条件、実動作条件を記載します。公式PDFが一次情報ですが、AIが実装・レビュー時にPDFの表を読み落とさないよう、必要な値を省略せず構造化します。
...
4. 書き込み対象ポートの「拡張ポートの入出力設定」が出力に設定されているか確認する。
5. 書き込むポートだけ、ポートの指示ビットを`1`にする。
6. ポートの設定値は、ポートの指示ビットが`1`のポートだけ反映されることを確認する。
7. 実機送信が必要な場合は、接続先、タイムアウト、ログ保存先、停止条件を決める。

## 6. PDF仕様フィールド定義

以下は、公式PDF `7.4.28` のコマンド表、レスポンス表、条件、例を、AIがカード単体で参照できるように転記したものです。値一覧、byte数、条件、ACK/NACK条件を省略せず、この節を実装時の一次確認に使ってください。

```text
7.4.28 拡張ポート値の書き込み
 リーダライタの「拡張ポート」の設定値を書き換えるコマンドです。
 なお、本コマンドで拡張ポート値の書き込みをおこなうためには、対象となる拡張ポートの「拡張ポ
...
(例) 拡張ポート 1 と 3 の値を変更し、拡張ポート 1=[0: Low]、拡張ポート 3=[1: High]
    とする場合
                            拡張ポート 8
...
※「ポートの指示」でビットが[1]になっていない拡張ポートは、 「ポートの設定値」に
...
上記設定とする場合、「ポートの指示」に[05]h、「ポートの設定値」に[04]h を指定します。
  ...
  ［コマンド／レスポンス例］
  • コマンド
    02 00 4E 03 A0 05 04 03 FF 0D

  •   レスポンス
      02 00 30 01 A0 03 D6 0D
```

## 7. コマンド形式の扱い

コマンド形式は、共通フレームとPDF 7.4.28のフィールド定義に従って実装してください。
...
| 4 | データ部[0] | 1 | `A0h` | 詳細コマンド。拡張ポート値の書き込み |
...
| bit | 対象 | 値`0` | 値`1` |
|---:|---|---|---|
| bit0 | 拡張ポート1 | 書き込まない | 書き込む |
| bit1 | 拡張ポート2 | 書き込まない | 書き込む |
| bit2 | 拡張ポート3 | 書き込まない | 書き込む |
| bit3 | 拡張ポート4 | 書き込まない | 書き込む |
| bit4 | 拡張ポート5 | 書き込まない | 書き込む |
| bit5 | 拡張ポート6 | 書き込まない | 書き込む |
| bit6 | 拡張ポート7 | 書き込まない | 書き込む |
| bit7 | 拡張ポート8 | 書き込まない | 書き込む |

### 7.3 データ部[2]: ポートの設定値

| bit | 対象 | 値`0` | 値`1` | 注意 |
|---:|---|---|---|---|
| bit0 | 拡張ポート1 | Low | High | データ部[1] bit0が`1`の場合のみ反映 |
| bit1 | 拡張ポート2 | Low | High | データ部[1] bit1が`1`の場合のみ反映 |
| bit2 | 拡張ポート3 | Low | High | データ部[1] bit2が`1`の場合のみ反映 |
| bit3 | 拡張ポート4 | Low | High | データ部[1] bit3が`1`の場合のみ反映 |
| bit4 | 拡張ポート5 | Low | High | データ部[1] bit4が`1`の場合のみ反映 |
| bit5 | 拡張ポート6 | Low | High | データ部[1] bit5が`1`の場合のみ反映 |
| bit6 | 拡張ポート7 | Low | High | データ部[1] bit6が`1`の場合のみ反映 |
| bit7 | 拡張ポート8 | Low | High | データ部[1] bit7が`1`の場合のみ反映 |

### 7.4 実動作条件

本コマンドは、アンテナポート8CH仕様のリーダライタの場合に有効です。また、拡張ポート値を書き込むには、対象ポートの入出力設定が「出力」に設定されている必要があります。
...
| 種別 | Hex |
|---|---|
| TX | `02 00 4E 03 A0 05 04 03 FF 0D` |
| RX | `02 00 30 01 A0 03 D6 0D` |

この例では、ポートの指示`05h`により拡張ポート1と3だけを書き込み対象にし、ポートの設定値`04h`により拡張ポート1をLow、拡張ポート3をHighへ変更します。
...
|---:|---|---|
| 0 | `detail_command` | `A0h`。拡張ポート値の書き込みに対するACK |

このACKには変更後ポート値の詳細データは含まれません。状態確認が必要な場合は `4Fh A0h` で読戻してください。
...
| DATA offset | フィールド | 解釈 |
|---:|---|---|
| 0 | `error_source` | 原則として対象コマンドの詳細識別子。対象: `4Eh A0h` |
...
このコマンドでは、上記に加えて、対象機がアンテナポート8CH仕様か、対象拡張ポートの機能、入出力設定、送信DATA[1]のポート指示、送信DATA[2]のポート設定値をパーサへ渡してください。
