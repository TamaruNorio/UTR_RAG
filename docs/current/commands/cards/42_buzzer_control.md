...
| `CMD=30h` かつ `DATA[0]` が `PDF該当節の先頭DATA` またはPDF該当節の応答識別子 | `ACK` | 対象コマンド `42h` の成功応答としてPDF該当節を読む |
| `CMD=6Ch` | `RF_TAG_DATA` | 自動読み取り中の非同期応答候補。通常ACKとは分離する |

対象識別子: コマンド `42h` / 詳細 `PDF該当節の先頭DATA` / サブ `なし`。


#### ACK/データ部offset
成功ACK `CMD=30h` のDATA先頭は、原則として `PDF該当節` またはPDF該当節の応答識別子として扱います。
