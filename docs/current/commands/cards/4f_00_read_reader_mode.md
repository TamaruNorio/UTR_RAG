...
このコマンドの目的は、**リーダライタ動作モードの読み取り** です。
...
| 項目 | 内容 |
|---|---|
| ACK構造 | `02 ADR 30 LEN 00 <READ_DATA...> 03 SUM 0D` |
| ACKデータ部の先頭 | `00h` |
| 後続データ | PDF 7.4.1 のACKレスポンス表で定義される読戻し値。バイト数と意味はコマンドごとに異なります。 |
...
| `CMD=30h` かつ `DATA[0]` が `00h` またはPDF該当節の応答識別子 | `ACK` | 対象コマンド `4Fh 00h` の成功応答としてPDF該当節を読む |
...
| DATA offset | フィールド | 解釈 |
|---:|---|---|
| 0 | `error_source` | 原則として対象コマンドの詳細識別子。対象: `4Fh 00h` |
