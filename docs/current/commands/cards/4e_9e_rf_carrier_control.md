...
- PDF section: 7.3.4
- Command name: RF送信信号の制御
- Category: リーダライタ制御
- Command byte: 4Eh
- Detail command byte: 9Eh
- Subcommand byte: -
...
- Reason: PDF Ver.1.17 lists this command in section 7.3.4. AI/RAG organizes device, ROM, parameter, impact, response, and recovery conditions instead of deciding arbitrary non-use.
...
- Parameter constraints: Follow the field definitions in PDF section 7.3.4.
...
- ACK command: Use the command-specific ACK structure described in PDF section 7.3.4.
- Data length: See PDF section 7.3.4.
...
- Command-specific NACK notes: Check PDF section 7.3.4 and section 7.6 together.
...
- Command format source:
  - PDF 7.3.4
- ACK response source:
  - PDF 7.3.4 ACK response
- NACK response source:
  - PDF 7.6 common NACK; PDF 7.3.4 NACK response
- Device/ROM support source:
  - PDF 6.2.1
- RAM/FLASH impact source:
  - NEEDS_RAM_FLASH_TRACE
- RF / carrier / antenna safety source:
  - PDF 3.1; PDF 7.3.4; docs/current/13_RF_SAFETY_AND_CARRIER_RULES.md
- Traceability status:
