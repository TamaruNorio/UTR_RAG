...
| 項目 | 内容 |
|---|---|
| ACK構造 | `02 ADR 30 04 <38h_or_48h> <param_type> <internal_ant> <external_ant> 03 SUM 0D` |
| `CMD` | `30h`（ACK） |
| `LEN` | `04h` |
| `DATA[0]` | PDF ACK表: `48h`、PDF掲載レスポンス例: `38h`。要実機確認 |
...
| 要確認 | PDFのACKレスポンス表では `DATA[0]=48h` と記載されていますが、掲載レスポンス例は `DATA[0]=38h` です。このカードでは原文差異を保持し、実機ログで最終確認してください。 |
...
| `CMD=30h` かつ `DATA[0]` が `38h` またはPDF該当節の応答識別子 | `ACK` | 対象コマンド `55h 38h` の成功応答としてPDF該当節を読む |
...
#### ACK/データ部offset
成功ACK `CMD=30h` は `LEN=04h` の設定反映型ACKです。PDF本文と掲載レスポンス例で `DATA[0]` に差異があるため、実機ログ確認までは `38h/48h` の両方を要確認候補として扱います。

| DATA offset | フィールド | 解釈 |
|---:|---|---|
| 0 | `detail` | PDF表: `48h`、PDF掲載例: `38h`。要実機確認 |
| 1 | `parameter_type` | `00h` コマンドモード / `01h` 自動読み取りモード / `02h` FLASH |
| 2 | `internal_antenna` | 内部アンテナ番号 |
| 3 | `external_antenna` | 外部アンテナ番号 |

`LEN=04h` を満たさない場合は、このコマンドの通常成功ACKとして扱わないでください。`DATA[0]` はPDF原文差異があるため、実機確認前に片方へ固定しないでください。
