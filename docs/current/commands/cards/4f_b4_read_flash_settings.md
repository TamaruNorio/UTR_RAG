...
このコマンドの目的は、**FLASH設定値の読み取り(1バイトアクセス)** です。
...
| 項目 | 内容 |
|---|---|
| ACK構造 | `02 ADR 30 LEN B4 <READ_DATA...> 03 SUM 0D` |
| ACKデータ部の先頭 | `B4h` |
| 後続データ | PDF 7.4.13 のACKレスポンス表で定義される読戻し値。バイト数と意味はコマンドごとに異なります。 |
...
| `CMD=30h` かつ `DATA[0]` が `B4h` またはPDF該当節の応答識別子 | `ACK` | 対象コマンド `4Fh B4h` の成功応答としてPDF該当節を読む |
