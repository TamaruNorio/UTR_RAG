...
| 項目 | 内容 |
|---|---|
| ACK構造 | `02 ADR 30 LEN 9F <READ_DATA...> 03 SUM 0D` |
| ACKデータ部の先頭 | `9Fh` |
| 後続データ | PDF 7.4.11 のACKレスポンス表で定義される読戻し値。バイト数と意味はコマンドごとに異なります。 |
...
| `CMD=30h` かつ `DATA[0]` が `9Fh` またはPDF該当節の応答識別子 | `ACK` | 対象コマンド `4Fh 9Fh` の成功応答としてPDF該当節を読む |
