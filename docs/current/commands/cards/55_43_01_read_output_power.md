...
- Status: SUPPORTED
- Reason: PDF Ver.1.17 lists this command in section 7.4.6. AI/RAG organizes device, ROM, parameter, impact, response, and recovery conditions instead of deciding arbitrary non-use.
...
- Parameter confirmation required: Target device series, ROM version, connection address, timeout policy, transmit output, carrier state, carrier timing
...
- Parameters: Target device series, ROM version, connection address, timeout policy, transmit output, carrier state, carrier timing
- Parameter constraints: Follow the field definitions in PDF section 7.4.6.
...
- ACK command: Use the command-specific ACK structure described in PDF section 7.4.6.
- Data length: See PDF section 7.4.6.
...
- Command-specific NACK notes: Check PDF section 7.4.6 and section 7.6 together.
...
- Required parameter confirmation: Target device series, ROM version, connection address, timeout policy, transmit output, carrier state, carrier timing
...
- Command format source:
  - PDF 7.4.6
- ACK response source:
  - PDF 7.4.6 ACK response
- NACK response source:
  - PDF 7.6 common NACK; PDF 7.4.6 NACK response
- Device/ROM support source:
  - PDF 6.2.2
- RAM/FLASH impact source:
  - PDF 7.4.6; docs/current/12_RAM_FLASH_IMPACT_MATRIX.md
- RF / carrier / antenna safety source:
  - PDF 3.1; PDF 7.4.6; docs/current/13_RF_SAFETY_AND_CARRIER_RULES.md
- Traceability status:
  - TRACE_COMPLETE
- Notes:
  - No completed Hex
...
- Latest result status:
  - REAL_DEVICE_PASS_WITH_NOTES
- Latest result notes:
  - v015 returned ACK using subcommand 01h and command-mode parameter kind 00h; no output power change was performed.
