...
このカードは、UTR-S201 シリーズ通信プロトコル説明書 Ver.1.17 に記載された **出力設定の読み取り** を、AIとの実装・レビュー・検証で参照しやすくするための整理資料です。
...
以下は、公式PDF `7.4.6` のコマンド表、レスポンス表、条件、例を、AIがカード単体で参照できるように転記したものです。値一覧、byte数、条件、ACK/NACK条件を省略せず、この節を実装時の一次確認に使ってください。

```text
7.4.6   出力設定の読み取り
  キャリア出力関係のパラメータを読み取るコマンドです。
...
データ長         1      0Bh
             ...
             2       100～240 1 [byte]目 ：下位バイト（LSB）
                             2 [byte]目 ：上位バイト（MSB）
データ部
                             キャリア送信時間 (msec) ［初期値：100］
                     1～4000
             2               1 [byte]目 ：下位バイト（LSB）
                       (※1)
                             2 [byte]目 ：上位バイト（MSB）
                             キャリア休止時間 (msec) ［初期値：50］
 ...
 SUM         1      SUM 値（「5.3 SUM の計算方法」参照）
  CR         1      0Dh
  ※取得した値の説明は、「7.4.16 出力設定の書き込み」をご参照ください。
  ※1：キャリア送信時間の設定可能範囲は、リーダライタの ROM バージョンにより
     異なります。
  ...
  ［コマンド／レスポンス例］
   • コマンド
      02 00 55 03 43 01 00 03 A1 0D
    • レスポンス
       02 00 30 0B 43 01 00 F0 00 64 00 32 00 C8 00 03 D2 0D
```
...
| 項目 | 内容 |
|---|---|
| ACK構造 | `02 ADR 30 0B 43 01 PARAM_KIND RF_POWER_TX[2] CARRIER_ON_MS[2] CARRIER_OFF_MS[2] CARRIER_SENSE_MS[2] 03 SUM 0D` |
| `LEN` | `0Bh` 固定 |
| `DATA[0]` | `43h`: 詳細コマンド |
| `DATA[1]` | `01h`: 詳細サブコマンド、出力設定の読み取り |
| `DATA[2]` | パラメータ種類。`00h`=コマンドモード、`01h`=自動読み取りモード、`02h`=FLASHデータ |
| `DATA[3..4]` | RF送信出力レベル。`dBm * 10`、LSBファースト |
| `DATA[5..6]` | キャリア送信時間msec。LSBファースト。ROMバージョンにより設定可能範囲が異なる |
| `DATA[7..8]` | キャリア休止時間msec。LSBファースト |
| `DATA[9..10]` | キャリアセンス待ち時間msec。LSBファースト |

このカードは読み取り専用ですが、値の意味は送信出力・キャリア時間に関わります。実装では読戻し値を表示・ログ保存に留め、書き込み系コマンドへ自動反映しないでください。
