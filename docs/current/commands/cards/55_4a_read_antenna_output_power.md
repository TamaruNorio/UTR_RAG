...
- Reason: PDF Ver.1.17 lists this command in section 7.4.15. AI/RAG organizes device, ROM, parameter, impact, response, and recovery conditions instead of deciding arbitrary non-use.
...
- Parameter confirmation required: Target device series, ROM version, connection address, timeout policy, connected antenna count, target antenna number, auto-switch range, transmit output, carrier state, carrier timing
...
- Parameters: Target device series, ROM version, connection address, timeout policy, connected antenna count, target antenna number, auto-switch range, transmit output, carrier state, carrier timing
- Parameter constraints: Follow the field definitions in PDF section 7.4.15.
...
- ACK command: Use the command-specific ACK structure described in PDF section 7.4.15.
- Data length: See PDF section 7.4.15.
...
- Command-specific NACK notes: Check PDF section 7.4.15 and section 7.6 together.
...
- Required parameter confirmation: Target device series, ROM version, connection address, timeout policy, connected antenna count, target antenna number, auto-switch range, transmit output, carrier state, carrier timing
...
- Command format source:
  - PDF 7.4.15
- ACK response source:
  - PDF 7.4.15 ACK response
- NACK response source:
  - PDF 7.6 common NACK; PDF 7.4.15 NACK response
- Device/ROM support source:
  - PDF 6.2.2
- RAM/FLASH impact source:
  - PDF 7.4.15; docs/current/12_RAM_FLASH_IMPACT_MATRIX.md
- RF / carrier / antenna safety source:
  - PDF 3.1; PDF 7.4.15; docs/current/13_RF_SAFETY_AND_CARRIER_RULES.md
- Traceability status:
  - TRACE_COMPLETE
- Notes:
  - No completed Hex
...
- v011 notes:
  - 8CH系またはROM条件依存の可能性。非対応なら NOT_APPLICABLE_TO_TARGET
  - 読み取りのみ。出力変更は対象外
- Related documents:
...
- Latest result status:
  - BLOCKED_BY_DEVICE_OR_ROM
- Latest result notes:
  - v015 did not send this command because it requires ROM 2.100 or later and the target ROM is 2.052.
