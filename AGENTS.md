...
- コマンドカードの `related_docs` に列挙されているファイルは、実装前にすべて開いて内容を確認してください。要約や記憶、以前のセッションでの理解に頼らないでください。
- NACK処理、複数アンテナ制御など、複数のコマンドカードにまたがる実装では、関係するすべてのカードとPDF該当節を横断的に確認してから実装してください。
- エラーコード、NACKコード、ステータスコードなど、仕様書に明記された固定値を実装する場合は、実装後に「この値はPDFのどの記述と一致するか」を1つずつ照合してください。根拠を示せない値は独自解釈せず、「未確認」として明記してください。
- 「タグ0枚」のような正常系の結果を、NACKや例外的なエラーコードで代用して判定しないでください。正常系と異常系は、PDFに明記された応答区分(ACK/NACK)の通りに実装してください。
